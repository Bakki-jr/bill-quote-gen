...
export default function Home() {
  ...
  const fetchIP = () => {
    axios.get("https://api.ipify.org/?format=json").then((res) => {
      setIsContentToBeDisplayed(allowedIPs.includes(res.data.ip));
  ...
  const calcuateCostPerFlavour = (flavours, inputsForPriceCalc) => {
    ...
    const flavourWithCalcPrice = flavours.map((item) => {
      return `${item.flavour}: ${cakeQuantity * Number(item.costPerKg) +
        noveltyCost +
  ...
  const getOutputTextContent = (data) => {
    ...
    const cakeModelType = `${data.cakeType ? `(${data.cakeType.toUpperCase()})` : ""
      }`;
  ...
  const getValidationOutput = (output) => {
    return (
      <div className="flex items-center justify-center h-[calc(100vh_-_.5rem)]  m-10 text-2xl">
        {output}
      </div>
    );
  };
  ...
  return (
    <div>
      ...
      ) : !isContentToBeDisplayed && !isIPVerified ? (
        getValidationOutput("Verifying IP...")
      ) : (
        getValidationOutput("Your IP is Restricted")
      )}
    </div>
  );
}
